...
import org.telegram.telegrambots.spring.tg.action.CommandValidator;
import org.telegram.telegrambots.spring.tg.action.TgAction;
import org.telegram.telegrambots.spring.tg.action.TgActionRequestHandler;
import org.telegram.telegrambots.spring.tg.action.ValidType;
 ...
 * @author enoy19
 * @see TgAction
 * @see org.telegram.telegrambots.spring.tg.action.request.TgRequest
		...
		return
				actions.stream()
						.filter(action -> {
							...
							ValidType bots = ValidType.NOT_EXISTING;

							if (action.isCommandValidatorExisting()) {
							...
							if(action.isBotsExisting()) {
								final TgContext tgContext = TgContextHolder.currentContext();
								final String absSenderName = tgContext.getAbsSenderName();
								bots = ValidType.NOT_VALID;

								for (String bot : action.getBots()) {
									if(bot.equals(absSenderName)) {
										bots = ValidType.VALID;
										break;
									}
								}
							}

							return commandValidatorVType.chainType(regexVType).chainType(bots).isValid();
						}).collect(Collectors.toList());
